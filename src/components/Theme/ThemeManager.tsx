...
const toSketchColor = (value: string) => {
  ...
  if (format === "rgba") return rgbaStringToHsva(value);
  if (format === "rgb") return rgbStringToHsva(value);
  return value || "#ffffff";
};
...
const ThemeManagerDialog = ({ open, onClose }: ThemeManagerDialogProps) => {
  ...
  const [draft, setDraft] = useState<ThemeDefinitionInput | null>(null);
  ...
  const handleSaveDraft = () => {
    ...
    setFeedback({
      type: "success",
      message: `${draft.name?.trim() || "Custom theme"} saved.`,
    });
    resetDraft();
  };
  ...
  const currentDraftColors: ThemeColors | null = draft ? draft[activeTab] : null;
    ...
    <Dialog open={open} onClose={onClose} fullScreen scroll="paper">
      ...
      <DialogContent dividers>
        ...
        <Typography variant="body2" sx={{ color: "text.secondary", mb: 2 }}>
          Manage presets, switch between them, or import/export JSON themes that
          are compatible with Qortal Hub and the Chrome Extension.
        </Typography>
        <Stack
