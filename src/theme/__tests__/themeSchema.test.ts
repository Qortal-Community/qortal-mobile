...
import { ThemeDefinition, defaultThemeDefinition } from "../../theme/themes";
...
describe("normalizeThemeImport", () => {
  it("accepts canonical Hub/Extension themes", () => {
  ...
  it("accepts legacy Go themes", () => {
    const legacy = createLegacyTheme();
    const normalized = normalizeThemeImport(legacy);
    expect(normalized.light.background).toBe(legacy.light.background);
  });
  ...
  it("throws descriptive error when palette missing", () => {
    expect(() =>
      normalizeThemeImport({
        name: "broken",
        light: null,
...
describe("serializeThemeDefinition", () => {
  it("exports canonical schema", () => {
    const payload = serializeThemeDefinition(defaultThemeDefinition);
    expect(payload.name).toBe(defaultThemeDefinition.name);
    expect(payload.light?.mode).toBe("light");
    expect(payload.dark?.primary?.main).toBe(defaultThemeDefinition.dark.primary);
